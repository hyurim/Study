...
오류를 발생시킬 가능성이 높거나 최적화 작업에 문제를 일으킬 수 있는 코드에 대해 명시적인 에러 발생시킴.

(하지만 `ESLint`가 더 효율적임.)

### 적용법

전역의 선두 및 함수 몸체의 선두에 `'use strict';`을 추가하면 됨.

1. 전역에 strict mode 적용하는 것은 피할 것.
   (외부 서드파티 라이브러리가 non-strict mode인 경우도 있기 때문에)

2. 함수 단위로 strict mode를 적용하는 것도 피할 것.
   (하나만 적용하는 것도 바람직하지 않고 하나하나 다 적용하는 것은 번거로움. <- 적용할 거면 즉시 실행함수로 감싼 스크립트 단위로 적용하는 것이 좋음.)
...
- 함수를 일반함수로 호출하면 this에 undefined가 바인딩됨.

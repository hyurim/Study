...
6. [이터레이터 패턴](#6-이터레이터-패턴iterator-pattern)
7. [노출모듈 패턴](#7-노출모듈-패턴revealing-module-pattern)
8. [MVC 패턴](#8-mvc-패턴)
9. [MVP 패턴](#9-mvp-패턴)
...
자바스크립트는 전역 범위에서 스크립트가 실행되는데 해당 패턴을 통해 접근 제어자를 구현하기도 함.

노출모듈 패턴을 기반으로 만든 ******CJS******(CommonJS) 모듈 방식이 있음.
...
> 재사용성이랑 확장성이 용이하면 뭐가 좋나요?
→ 기존 시스템을 그대로 사용할 수 있으며(시간과 비용 절감), 재설계 / 재설치 등의 필요 없다 !

![](https://velog.velcdn.com/images/hyuri/post/6051d7da-694b-420d-bef6-a8143cc62f43/image.png)


### 모델

애플리케이션의 데이터인 DB, 상수, 변수 등을 뜻 함.

뷰에서 데이터 생성하거나 수정 → 컨트롤러 → 모델 생성 또는 갱신

### 뷰

사용자 인터페이스 요소, 즉 모델을 기반으로 사용자가 볼 수 있는 화면을 뜻 함.
...
하나 이상의 모델과 뷰를 잇는 다리 역할을 하며 이벤트 등 메인 로직을 담당함.

모델과 뷰의 생명주기도 관리 함

모델이나 뷰의 변경통지를 받으면 이를 해석해서 각각의 구성 요소에 해당 내용에 대해 알려줌.

### MVC 패턴의 예

대표적인 라이브러리로는 React가 있다.

유저 인터페이스를 구축하기 위한 라이브러리이며, 가상 DOM을 통해 실제 DOM을 조작하는 것을 추상화해서 성능을 높였음.

---

## 9. MVP 패턴

MVC 패턴의 파생으로 MVC의 C가 프레젠터(Presenter)로 교체된 패턴

뷰와 프레젠터는 일대일 관계이기 때문에 MVC 패턴보다 더 강한 결합을 지녔다.

![](https://velog.velcdn.com/images/hyuri/post/9b82bc4a-8aca-417b-a298-6c774f8b2e2b/image.png)
...
MVC 패턴에서 C가 뷰 모델(View Model)로 바뀐 패턴

뷰모델 : 뷰를 더 추상화한 계층

차이점은 해당 패턴은 커맨드와 데이터 바인딩을 가짐.
...
![](https://velog.velcdn.com/images/hyuri/post/f2314065-0f6a-4047-b51f-99c0e7c7fa80/image.png)


### 장점

1. 뷰와 뷰모델 사이의 양방향 데이터 바인딩을 지원
2. UI를 별도의 코드 수정 없이 재사용할 수 있음.
3. 단위 테스팅하기 쉬움

> Ex) **Vue.js**<br>
반응형이 특징인 프론트엔드 프레임워크
값 대입만으로도 변수가 변경되며, 양방향 바인딩, html을 토대로 컴포넌트를 구축할 수 있다.